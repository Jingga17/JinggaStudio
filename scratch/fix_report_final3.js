...
const idx1 = code.lastIndexOf(target1);
if (idx1 !== -1) {
  code = code.substring(0, idx1) + inject1 + "\\n" + code.substring(idx1);
  console.log("Injected renderIndividuReal");
}
...
const target2 = "      let html = `\n        ${this.getKopSurat(`LAPORAN ANALISIS KELAS<br>KELAS ${kelas}`)}";
const idx2 = code.lastIndexOf(target2);
if (idx2 !== -1) {
  code = code.substring(0, idx2) + inject2 + "\\n" + code.substring(idx2);
  console.log("Injected renderKelasReal");
}
